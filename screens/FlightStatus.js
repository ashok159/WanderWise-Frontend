...
// av.addListener(() => {return});
//https://airlabs.co/api/v9/flight?flight_iata=IB2627&api_key=fd5bb2e1-ffd0-46bc-a128-0fa2cb205c4c
const FlightStatus = () => {
  ...
  const getFlightData = async (flightIataIn) => {
    const api_key = 'fd5bb2e1-ffd0-46bc-a128-0fa2cb205c4c'
    try {
      // const url = `http://${Platform.OS === "ios" ? "localhost" : "10.0.2.2"}:8000/api/flight-status/${flightIataIn}/`;
      const url = `https://airlabs.co/api/v9/flight?flight_iata=${flightIataIn}&api_key=fd5bb2e1-ffd0-46bc-a128-0fa2cb205c4c`
      const res = await fetch(url);
      const json = await res.json();
      console.log(json)

      // if (json.status === 'success') {
      //   onChangeFlightObj(json.data);
      //   setVisibility(false);
      // } else {
      //   setVisibility(true);
      // }
    ...
    <View style={styles.container}>
      <View style={styles.flightTitleContainer}>
        <Text style={styles.flightTrackerTitle}>Flight Status</Text>
          {/* <Image source={require('frontend/assets/icons/black-plane.png')} resizeMode='contain' style={styles.planeImage}></Image> */}
      </View>
