...
function BudgetTracker() {
    ...
    const checkHasBudget = async (firebaseUserID) => {
        try {
            const url = `https://ashok159.pythonanywhere.com/check_max_budget/${firebaseUserID}/`; 
    ...
    const createOrUpdateMaxBudget = async () => {
        if (isNaN(maxBudgetString) === false) {
            try {
                ...
                const url = `https://ashok159.pythonanywhere.com/check_max_budget/${auth.currentUser.uid}/`;
                const response = await axios.get(url);
    
                if (response.data.max_budget !== null) {
                    // Update the existing budget for the user
                    const updateURL = `https://ashok159.pythonanywhere.com/update_max_budget/${auth.currentUser.uid}/`;
                    ...
                    const createURL = 'https://ashok159.pythonanywhere.com/api/budgets/';
                    const createData = {
    ...
    const addEvent = async (eventData) => {
        if (eventData.event_name === '' || eventData.event_description === '' || eventData.event_location === '' || eventData.event_budget === ''){
            ...
            try {
                ...
                const url = `https://ashok159.pythonanywhere.com/api/events/`;
    ...
    const setEventsList = async (firebaseUserID) => {
        try {
            ...
            const url = `https://ashok159.pythonanywhere.com/get_events/${firebaseUserID}/`;
            const response = await axios.get(url);
    ...
    const deleteEvent = async (firebaseUserID, itemID) => {
        try{
            // console.log(firebaseUserID, itemID);
            // const url = `http://${Platform.OS === "ios" ? "localhost" : "10.0.2.2"}:8000/delete_event/${firebaseUserID}/${itemID}/`;
            const url = `https://ashok159.pythonanywhere.com/delete_event/${firebaseUserID}/${itemID}/`;
