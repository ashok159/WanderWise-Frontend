...
const OtherUserPosts = ({ navigation, userDataIn, userUID }) => {
  ...
  const timeSincePost = (data) => {
   ...
   // console.log(`Time elapsed: ${days} days, ${hours} hours, ${minutes} minutes, ${seconds} seconds`);
  ...
  const fetchLikedStatus = async () => {
    try {
      ...
      // console.log(likedData.liked && likedData.liked.includes(auth.currentUser.uid))
  ...
  const handleUserProfileClick = async () => { // Access the current route
  ...
  return (
    <View style={styles.postContainer}>
      <View style={styles.postHeader}>
        ...
        <View style={styles.userInfo}>
          ...
          {userDataIn.location ? (
            ...
            >
              {/* <FontAwesome5 name="map-marker" size={12} color="#007BFF" /> */}
      ...
      {/* Like and Comment Section uri: userDataIn.URL */}
      {/* <View style={styles.likeCommentSection}>
        <TouchableOpacity style={styles.likeCommentButton} onPress={toggleLike}>
            <FontAwesome5 name="heart" size={24} color={liked ? 'red' : '#333'} />
            <Text style={styles.likeCommentText}> {likes} </Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.likeCommentButton}>
          <FontAwesome5 name="comment" size={24} color="#333" />
          <Text style={styles.likeCommentText}> {numberOfComments}  </Text>
        </TouchableOpacity>
      </View> */}
