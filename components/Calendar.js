...
function MyCalendar() {
  ...
  const handleAddEvent = () => {
    // Reset input fields when adding a new event
    setEventName("");
    setLocationName("");
    setEventTime("");
    setEventDescription("");
  ...
  const deleteFavLoc = async () => {
    // const userRef = doc(db, 'User Calendars', auth.currentUser.uid);
    // const userEventRef = doc(userRef, 'Events', index);
    const userUploadsCollectionRef = collection(db, "User Calendars");
    ...
    <View style={styles.container}>
      ...
      >
...

        {
                ...
                <View key={index} style={styles.eventContainer}>
                  ...
                  >
                    ...
                    <View style={{ flexDirection: "row", alignItems: "center" }}>
                      {/*
                     <TouchableOpacity onPress={() => handleEditEvent(item)} style={{ marginRight: 9 }}>
                     <FontAwesome5 name="edit" style={{ fontSize: 18, color: 'black' }} />
                     </TouchableOpacity>
                      */}
                    <TouchableOpacity
                  ...
                  {/* <TouchableOpacity style={styles.eventDescription}><Text>{item.eventLocation}</Text></TouchableOpacity> */}
          ...
          !isLoading ? 
          <View>
            {Object.keys(groupedUserEvents).map(date => (
              <View key={date}>
                <Text style={{ fontSize: 20, fontWeight: 'bold' }}>{date}</Text>
                {groupedUserEvents[date].map(event => (
                  <View key={event.id} style={{ marginLeft: 10 }}>
